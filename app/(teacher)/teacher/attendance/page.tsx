...
import { Calendar, Users, CheckCircle, XCircle, Clock, Save, ArrowLeft, Loader2 } from "lucide-react";
...
function AttendanceContent() {
    ...
    useEffect(() => {
        if (allocationIdParam) {
            fetchAllocationDetails(allocationIdParam);
        } else {
    ...
    const fetchTeacherAllocations = async () => {
        ...
        if (userDataStr) {
            ...
            try {
                // 1. Get Teacher ID
                const teacherRes = await fetch(`/api/teachers?email=${userData.email}`);
                if (teacherRes.ok) {
                    ...
                    if (teachers.length > 0) {
                        ...
                        if (allRes.ok) {
                            ...
                            // Loose comparison
                            const myAllocations = all.filter((a: any) => {
                            ...
                            if (myAllocations.length === 0) {
                                setDebugInfo(`Teacher ID: ${teacherId}, Total Allocations: ${all.length}. No matches found.`);
                            }
                        } else {
                            setDebugInfo("Failed to fetch allocations API");
                        }
                    } else {
                        setDebugInfo(`No teacher profile found for email: ${userData.email}`);
                    }
                } else {
    ...
    const fetchAllocationDetails = async (id: string) => {
        try {
            ...
            if (allRes.ok) {
                const all = await allRes.json();
                const found = all.find((a: any) => a.id === parseInt(id));
                if (found) {
                    setAllocation(found);
    ...
    const fetchStudents = async (batch: string, section: string) => {
        try {
            const res = await fetch(`/api/students?batch=${batch}&section=${section}`);
            if (res.ok) {
    ...
    const saveAttendance = async () => {
        ...
        if (unmarked > 0) {
            if (!confirm(`Warning: ${unmarked} students are not marked. Save anyway?`)) return;
        }
            ...
            <div className="space-y-6 animate-fade-in">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-muted-foreground">Select a class to proceed</p>
                    </div>
                </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Choose one of your assigned classes</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {allocations.length > 0 ? (
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                {allocations.map((alloc) => (
                                    <div
                                        ...
                                        onClick={() => handleAllocationSelect(alloc.id.toString())}
                                    >
                                        <div className="font-bold text-lg text-primary">{alloc.course.code}</div>
                                        <div className="text-sm font-medium">{alloc.course.name}</div>
                                        <div className="flex gap-2 mt-2">
                                            <Badge variant="secondary">{alloc.batch}</Badge>
                                            <Badge variant="outline">Sec {alloc.section}</Badge>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="text-center py-8 text-muted-foreground">
                                ...
                                <Link href="/teacher/dashboard"><Button className="mt-4" variant="outline">Back to Dashboard</Button></Link>
                            </div>
                        )}
                    </CardContent>
                </Card>
            ...
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        {allocation ? `${allocation.course.code} - ${allocation.batch} (${allocation.section})` : "Loading..."}
                    </p>
                </div>
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={markAllPresent}>
                            <CheckCircle className="h-4 w-4 mr-2" /> Mark All Present
                        </Button>
                        <Button onClick={saveAttendance} disabled={saving} className="bg-green-600 hover:bg-green-700 text-white">
                        ...
                        <table className="w-full caption-bottom text-sm text-left">
                            <thead className="[&_tr]:border-b">
                                <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                    <th className="h-12 px-4 align-middle font-medium text-muted-foreground">Student ID</th>
                            ...
                            <tbody className="[&_tr:last-child]:border-0">
                                {students.map((student) => (
                                    <tr key={student.id} className="border-b transition-colors hover:bg-muted/50">
                                        <td className="p-4 align-middle font-mono">{student.studentId}</td>
                                        ...
                                        <td className="p-4 align-middle">
                                            <div className="flex gap-2">
                                                <Button
                                                    ...
                                                    className={`w-24 ${student.status === "Present" ? "bg-green-600 hover:bg-green-700" : "opacity-50 hover:opacity-100"}`}
                                                ...
                                                <Button
                                                    ...
                                                    className={`w-24 ${student.status === "Absent" ? "" : "opacity-50 hover:opacity-100"}`}
                                                ...
                                                <Button
                                                    ...
                                                    className={`w-24 ${student.status === "Late" ? "bg-yellow-500 text-white" : "opacity-50 hover:opacity-100"}`}
