...
export function Header() {
    ...
    const roleLabel = isAdmin ? "Administrator" : isTeacher ? "Teacher" : isStudent ? "Student" : "User";
    const profileHref = isAdmin ? "/admin/profile" : isTeacher ? "/teacher/profile" : isStudent ? "/student/profile" : "/profile";
    ...
    const userEmail = user?.email || (isAdmin ? "admin@system.com" : "user@school.edu");
                        ...
                        <div className="max-h-[300px] overflow-y-auto">
                            {notifications.length > 0 ? (
                                notifications.map((notification: Notification) => (
                                    <DropdownMenuItem key={notification.id} className="cursor-pointer" onClick={() => markAsRead(notification.id)}>
                                        <div className={`flex flex-col gap-1 ${!notification.isRead ? 'font-semibold' : 'text-muted-foreground'}`}>
